...
class Filament: 
    def __init__(self, path_to_filament_NDskl, dimensions='3D'):
    ...
    def import_fils(self):
        ...
        if self.dimensions =='3D':
            ...
            filaments_dm = rf.ReadFilament2D(skeleton_file_dm)
    ...
    def filament_coordinates(self):
        ...
        for i in range(self.filament_dict['nfils']):
            if self.dimensions=='3D':
                ...
                p = self.filament_dict['filaments'][i]['px,py']
                p = pd.DataFrame(p,columns = ['px', 'py'])
            p['Filament ID'] = i
            df = pd.concat([df, p])
        return df
    ...
    def segment_coordinates(self):
        ...
        for i in range(self.filament_dict['nfils']):
            if self.dimensions=='3D':
                ...
                p = np.array(self.filament_dict['filaments'][i]['px,py'])
                p = np.hstack([p[:-1],p[1:]])
                p = pd.DataFrame(p,columns = ['px', 'py','px2','py2'])
            p['Filament ID'] = i
        ...
        for i in range(N):
            ...
            if C == cp_id:
                if self.dimensions=='3D':
                    ...
                    cp_list.append([critical_points[i]['px'],critical_points[i]['py']])
