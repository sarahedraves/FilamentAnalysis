...
def clip_polyline_to_box(points, bounds):
    """
    Vectorized clipping of a polyline to a 3D axis-aligned box.

    points: (N,3) array
    bounds: (xmin, xmax, ymin, ymax, zmin, zmax)

    Returns arrays of clipped segment endpoints.
    """
    ...
    for dim in range(3):
        ...
        parallel = np.abs(d_dim) < 1e-12

        # Reject parallel segments outside slab
        outside_parallel = parallel & (
            (p0_dim < bounds_min[dim]) |
            (p0_dim > bounds_max[dim])
        )

        t1 = (bounds_min[dim] - p0_dim) / d_dim
        t2 = (bounds_max[dim] - p0_dim) / d_dim

        t_low = np.minimum(t1, t2)
        t_high = np.maximum(t1, t2)

        tmin = np.where(parallel, tmin, np.maximum(tmin, t_low))
        tmax = np.where(parallel, tmax, np.minimum(tmax, t_high))

        reject = outside_parallel | (tmin > tmax)
    ...
    new_p0 = p0[valid] + (tmin[valid][:, None] * d[valid])
    new_p1 = p0[valid] + (tmax[valid][:, None] * d[valid])
...
def plot(filament_idx, filament_dict, ax, colorfil="teal"):
    ...
    points = np.asarray(cords[:sample])
    ...
    clipped_p0, clipped_p1 = clip_polyline_to_box(points, bounds)

    if len(clipped_p0) == 0:
        return

    # Build segments array (N, 2, 3)
    segments = np.stack([clipped_p0, clipped_p1], axis=1)
...
# def plot(filament_idx, filament_dict,ax,colorfil="teal"):
#     """
#     This function will plot fillaments given the filament index and filament dictionary
#     ======================================================================================
#     Paremeter
...
def plot_2D(filament_idx, filament_dict,L=1,colorfil="black",offset=0.5,a=0.5):
    """
    This function plots filaments in 2D atop a gas cube centered at 0.5
    ============================================================================
    Parameters
    ...
    """
    sample=filament_dict[filament_idx]["nsamp"] 
    cords=filament_dict[filament_idx]["px,py"]
    ...
    fil = plt.plot(px,py,c=colorfil,lw='2',alpha=a) 


def plot_3Das2D(filament_idx, filament_dict,L=1,colorfil="black",offset=0.5,axes='xy'):
    """
    This function plots filaments in 2D atop a gas cube centered at 0.5
    ============================================================================
    Parameters
    ...
    pb= []
    for i in range(sample):
        pa1 =L*(cords[i][axisa]- offset)
        pb1 =L*(cords[i][axisb]- offset)
        
        pa.append(pa1)
        pb.append(pb1)
        
    fil = plt.plot(pa,pb,c=colorfil,lw='2',alpha=0.5) 
